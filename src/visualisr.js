...
Visualisr = function(canvas) {
	...
	// Run on construction
	Visualisr.helpers.init(this.context);
};
...
Visualisr.defaults = {
	...
	color: {
		...
		border: "#333",		// canvas border colour
 ...
 */
Visualisr.helpers.init = function(context) {
	context.canvas.style.backgroundColor = Visualisr.defaults.color.bg;
	
	Visualisr.helpers.applyPixelRatio();
	Visualisr.helpers.makeFontStrings();
	
	if (Visualisr.defaults.global.fillParent) { Visualisr.helpers.fillParent(context); }
};
 ...
 * 		- context: Canvas context for canvas we're applying Visualisr.js to
 */
Visualisr.helpers.fillParent = function(context) {
	...
	context.translate(padding, Math.floor(canvas.height - padding));
	
	// bind window resize
	bindWindowResize(context);
};
...
function bindWindowResize(context) {
	...
	function resizeEnd() {
		if (new Date() - rtime < rwait) {
			...
			timeout = false;
			Visualisr.helpers.fillParent(context);
		}
	}
}
